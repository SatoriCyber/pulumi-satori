...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `satori` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `SatoriCyber` with the `ORG` of your provider.
   - replace all instances of the `github.com/SatoriCyber/pulumi-satori` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-satori
  ...
  ignore: []
  ldflags:
  - -X github.com/SatoriCyber/pulumi-satori/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-satori/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-satori_darwin_amd64_v1
│   └── pulumi-resource-satori
├── pulumi-satori_darwin_arm64
│   └── pulumi-resource-satori
├── pulumi-satori_linux_amd64_v1
│   └── pulumi-resource-satori
├── pulumi-satori_linux_arm64
│   └── pulumi-resource-satori
├── pulumi-satori_windows_amd64_v1
│   └── pulumi-resource-satori.exe
└── pulumi-satori_windows_arm64
    └── pulumi-resource-satori.exe
```
